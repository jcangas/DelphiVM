...
class Registry < DvmTask
  ...
  def copy(idever=nil)
    ...
    tt_path  = (options[:template] ? ide.prj_regkey(options[:template]).upcase : regkey.parent.basename)

    regkey = regkey.parent.parent

    source = Pathname(tt_path) + version
    ...
    say WinServices.reg_copy(source: (Pathname('hkcu') + regkey + source).win, dest: (Pathname('hkcu') + regkey + dest).win, recurse: true, force: true)
  end
